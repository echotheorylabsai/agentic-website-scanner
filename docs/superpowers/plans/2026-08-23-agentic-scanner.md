...
- Canonical frame order — **the live capture (`sse-fresh.txt`) is the source of truth**: `kind_detecting → kind_detected → scan_init{roster+layerMaxScores} → scan_init{totalChecks,staticOnly} → discovery_phase×8 → check_start batched (84 up front, not strictly interleaved) → check_complete frames as they resolve (124 complete for a 104 totalChecks counter — progress UI must clamp) → layer_complete×4 → scan_complete{pre-gating result object} → discovery_phase×1 → relevance_assessed{final} → discovery_phase×2 (summary → archive) → summary_ready → scan_archived`. Frames are flat `{type,...}` JSON on `data:` lines; `check_complete` may carry `bonus:true`; `layer_start` never emitted. Stream endpoint starts scans when missing; `scan_archived` only after DB commit.
